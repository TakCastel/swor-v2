...
export class ForumDataService {
  ...
  private readonly MOCK_POSTS: Post[] = [
  ...
  private readonly MOCK_TOPICS: Topic[] = [
    { id: 1, title: '[Mission] Briefing on Christophsis', author: 'JediMasterValorum', replies: 25, views: 1230, lastPostInfo: 'by KnightTano, 2 hours ago', posts: this.MOCK_POSTS.slice(0, 2) },
    { id: 2, title: 'Infiltration of the Imperial Command', author: 'CaptainEvaRex', replies: 42, views: 2500, lastPostInfo: 'by RogueSpectre, 5 hours ago', posts: [this.MOCK_POSTS[2]] },
  ...
  private readonly MOCK_FORUMS: Forum[] = [
      // Clone Wars
      { id: 101, name: 'Coruscant - Galactic Senate', description: 'Diplomacy, intrigue, and political maneuvering in the heart of the Republic.', topicsCount: 15, postsCount: 302, lastPostInfo: 'by SenatorOrgana, 1h ago', topics: [this.MOCK_TOPICS[0]] },
      ...
      // Civil War
      { id: 201, name: 'Rebel Alliance - Secret Bases', description: 'Plan strikes, gather intel, and fight for freedom from hidden locations.', topicsCount: 31, postsCount: 812, lastPostInfo: 'by CaptainEvaRex, 5h ago', topics: [this.MOCK_TOPICS[1]] },
      ...
      { id: 302, name: 'Imperial Remnant Warlords', description: 'Carve out your own fiefdom in the chaotic Outer Rim.', topicsCount: 25, postsCount: 680, lastPostInfo: 'by GrandMoffGideon, 4h ago', topics: [this.MOCK_TOPICS[3]] },
  ];

  private readonly MOCK_DATA: Category[] = [
    {
      id: 1,
    ...
    },
    {
      ...
      forums: this.MOCK_FORUMS.filter(f => f.id >= 101 && f.id <= 199),
    },
    {
      id: 3,
      name: 'La Guerre Civile Galactique',
      forums: this.MOCK_FORUMS.filter(f => f.id >= 201 && f.id <= 299),
    },
    {
      id: 4,
      name: "L'Après-Empire (Légendes)",
      forums: this.MOCK_FORUMS.filter(f => f.id >= 301 && f.id <= 399),
    },
     {
      id: 5,
      name: 'Hors-Jeu',
  ...
  getForumById(id: number) {
    for (const category of this.MOCK_DATA) {
      const forum = category.forums.find(f => f.id === id);
      if (forum) return of(forum);
    }
    return of(undefined);
  }

  getTopicById(id: number) {
     for (const topic of this.MOCK_TOPICS) {
         if(topic.id === id) return of(topic);
     }
     return of(undefined);
  }
}
